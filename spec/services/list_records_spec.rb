...
RSpec.describe ListRecords do
  ...
  describe '#call' do
    subject { ListRecords.new(domain) }
    ...
    it "calls Aws::Route53::Client#list_resource_record_sets" do
      subject.call
      expect(route53_client).to have_received(:list_resource_record_sets).with(hosted_zone_id: domain.route53_hosted_zone_id)
    end
